...
def start_game():
    ...
    2. You will get feedback for each letter, with colors:
       \033[92m✓\033[0m = correct letter in the correct spot (green)
...
def check_letters(secret_word, guess):
    ...
    # Track how many of each letter are in the secret word
    ...
    for i in range(5):
        if guess_letters[i] == secret_letters[i]:
            top_row += f"\033[92m{guess_letters[i]}\033[0m "  # Green
            bottom_row[i] = f"\033[92m✓\033[0m"
    ...
    for i in range(5):
        if bottom_row[i] == "":
            # Only mark as yellow (~) if the letter exists in the word and hasn’t been used yet
            ...
            if guess_letters[i] in secret_count and secret_count[guess_letters[i]] > 0:
                ...
                top_row[i] = f"{color}{guess_letters[i]}\033[0m"
...
def valid_guess(word_list):
    ...
    and hasn’t been guessed already.
    """
    words_guessed = []
    valid_guess = False
    while not valid_guess:
        ...
        if len(guess) != 5:
            ...
            print("That’s not a valid word in our dictionary, please try again.")
...
def game_loop():
    ...
    if guess != secret_word:
        print(f"😢 Sorry, you didn't guess the word: {secret_word}")
    play_again()
